...
/*   Updated: 2026/04/05 14:57:13 by lomont           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void server::CreateNewClient(int newConnexion, struct sockaddr sockaddrClient, socklen_t socklenClient, int ServerSocket)
{
	...
	EV_SET(tevent, newConnexion, EVFILT_READ, EV_ADD | EV_ENABLE, 0, 0, 0);
	if (kevent(EvenementQueue, tevent, 1, NULL, 0, NULL) == -1) {
		close(newConnexion);
...
void server::WaitForConnection(void)
{
	...
	{																													// laisser tourner le serveur tout le temps
		CheckTimestamp();
		if ((nbOfEvents = kevent(this->EvenementQueue, NULL, 0, this->tevent, SIZE_TEVENT, &kqueue_timeout)) == -1) // si on recoit -1 de kevent alors crash => on stock dans nbOfEvents
			ft_logs("Triggered event retrieval error");
		for (int i = 0; i < nbOfEvents; i++)
		{ // pour nombres d'events triggered
			if ((serverSocket = this->findServerSocket(this->tevent[i].ident)) != -1)
			...
			{
				current = FindCurrentClient(static_cast<int>(this->tevent[i].ident)); // on cherche le client associé au fd retourné par kevent => ce qui veut dire qu'un client nous a contacté
				if (!current)															  // si on a rien on continue
					continue;
				current->RefreshTimestamp();
				if (this->tevent[i].filter == EVFILT_READ)
					current->ReceiveHeader(this->EvenementQueue, map);
				else if (this->tevent[i].filter == EVFILT_WRITE)
...
int server::ConfigureServer(void)
{
	...
	if (EvenementQueue == -1) {
		ft_error("kqueue fd creation failed");
		return (-1);
	}
